...
import type { FieldConfig, SectionConfig, TemplateConfig } from '../../templates/types'
...
export function EditorScreen() {
	...
	const styleOverrides = (invitation.designOverrides ?? {}) as Record<string, string>
	...
	}, [previewRef.current, draft, sectionVisibility])
	...
	const activeSectionConfig: SectionConfig | undefined = template?.sections.find(
	...
	const handleAiApply = () => {
		...
		if (aiPanel.type === 'style') {
			...
			next.schedule.events = (aiPanel.result.events ?? []) as InvitationContent['schedule']['events']
			updateDraft(next)
		} else if (aiPanel.sectionId === 'faq') {
			next.faq.items = (aiPanel.result.items ?? []) as InvitationContent['faq']['items']
			updateDraft(next)
		} else if (aiPanel.sectionId === 'story') {
			next.story.milestones = (aiPanel.result.milestones ?? []) as InvitationContent['story']['milestones']
			updateDraft(next)
	...
	const renderListEditor = (sectionId: string) => {
		...
		const listValue = (getDefaultSection(sectionId, draft) ?? {}) as any
		const items =
			sectionId === 'story'
		...
		const updateItems = (nextItems: any[]) => {
			const next = structuredClone(draft)
			if (sectionId === 'story') next.story.milestones = nextItems
			if (sectionId === 'schedule') next.schedule.events = nextItems
			if (sectionId === 'faq') next.faq.items = nextItems
			if (sectionId === 'gallery') next.gallery.photos = nextItems
			if (sectionId === 'entourage') next.entourage.members = nextItems
			updateDraft(next)
		}
						...
						{listConfig.fields.map((field) => (
							<label key={field.key} className="grid gap-1 text-xs uppercase tracking-[0.2em] text-[color:var(--dm-muted)]">
								...
								<input
									...
									onChange={(event) => {
										const nextItems = safeItems.map((current: any, itemIndex: number) =>
											itemIndex === index
							...
							<div className="mt-4 space-y-4">
								...
								<div className="mt-6 space-y-4">
									{activeSectionConfig?.fields.map((field) => (
										<div key={field.id}>{renderField(activeSectionConfig.id, field)}</div>
									))}
								</div>
							</div>
						</div>
