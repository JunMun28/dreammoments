...
describe("CE-029: AnimationProperties", () => {
	...
	describe("Transform origin picker", () => {
		it("renders transform origin section", () => {
			render(<AnimationProperties object={createMockObject()} />);

			const originLabel = screen.getByText(/transform origin/i);
			expect(originLabel).toBeDefined();
		});

		it("renders 9-point origin picker grid", () => {
			render(<AnimationProperties object={createMockObject()} />);

			const originPicker = screen.getByTestId("origin-picker");
			expect(originPicker).toBeDefined();

			// Should have 9 buttons (3x3 grid)
			const buttons = originPicker.querySelectorAll("button");
			expect(buttons.length).toBe(9);
		});

		it("highlights center origin by default", () => {
			render(<AnimationProperties object={createMockObject()} />);

			const centerButton = screen.getByTestId("origin-center");
			expect(centerButton.getAttribute("aria-pressed")).toBe("true");
		});

		it("highlights current origin from config", () => {
			render(
				<AnimationProperties
					object={createMockObject({ origin: "top-left" })}
				/>,
			);

			const topLeftButton = screen.getByTestId("origin-top-left");
			expect(topLeftButton.getAttribute("aria-pressed")).toBe("true");
		});

		it("calls onPropertyChange when origin is changed", () => {
			const onPropertyChange = vi.fn();
			render(
				<AnimationProperties
					object={createMockObject()}
			...
			const topRightButton = screen.getByTestId("origin-top-right");
			topRightButton.click();

			expect(onPropertyChange).toHaveBeenCalledWith(
				"animation",
				expect.objectContaining({ origin: "top-right" }),
			);
		});

		it("includes all 9 origin options", () => {
			render(<AnimationProperties object={createMockObject()} />);
			...
			for (const origin of origins) {
				const button = screen.getByTestId(`origin-${origin}`);
