...
export function FabricCanvas({
	...
	const checkAlignmentGuides = useCallback(
		(movingObject: FabricObject) => {
			...
			// Track snap positions - null means no snap, value means snap to that position
			...
			if (Math.abs(movingRight - CANVAS_WIDTH) < ALIGNMENT_THRESHOLD) {
				showVerticalGuide = true;
				verticalGuideX = CANVAS_WIDTH;
				snapLeft = CANVAS_WIDTH - movingWidth;
			}
			...
			if (Math.abs(movingBottom - CANVAS_HEIGHT) < ALIGNMENT_THRESHOLD) {
				showHorizontalGuide = true;
				horizontalGuideY = CANVAS_HEIGHT;
				snapTop = CANVAS_HEIGHT - movingHeight;
			}
