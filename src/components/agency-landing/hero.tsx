		...
		() => ({
			...
			iResolution: {
				value: new THREE.Vector2(
					window.innerWidth * Math.min(window.devicePixelRatio, 0.5),
					window.innerHeight * Math.min(window.devicePixelRatio, 0.5),
				),
			},
			isDark: { value: 1.0 },
			iScroll: { value: 0.0 },
			color1Start: {
				value: new THREE.Vector3(
					...hexToRgb(startColors?.color1 ?? defaultStart.color1),
				),
			},
			color2Start: {
				value: new THREE.Vector3(
					...hexToRgb(startColors?.color2 ?? defaultStart.color2),
				),
			},
			color3Start: {
				value: new THREE.Vector3(
					...hexToRgb(startColors?.color3 ?? defaultStart.color3),
				),
			},
			color1End: {
				value: new THREE.Vector3(
					...hexToRgb(endColors?.color1 ?? defaultEnd.color1),
				),
			},
			color2End: {
				value: new THREE.Vector3(
					...hexToRgb(endColors?.color2 ?? defaultEnd.color2),
				),
			},
			color3End: {
				value: new THREE.Vector3(
					...hexToRgb(endColors?.color3 ?? defaultEnd.color3),
				),
			},
		}),
...
export function Hero({ startColors, endColors }: HeroProps = {}) {
	...
	return (
		...
		>
			...
			<div
				className="relative z-10 mx-auto flex h-full max-w-[90rem] flex-col justify-start pt-44 px-6 text-left sm:px-12 sm:pt-48 md:justify-center md:pt-0 lg:px-24 2xl:max-w-[112.5rem] min-[120rem]:max-w-[137.5rem]"
			...
			>
				<h1 className="text-[clamp(3rem,8vw,12rem)] leading-[1.05] tracking-tight text-foreground">
					...
					<span className="block overflow-hidden pb-[0.1em]">
						...
						>
							<em className="font-serif">your love story.</em>
						</motion.span>
					</span>
				</h1>
			...
			<motion.div
				className="absolute bottom-8 left-1/2 z-10 -translate-x-1/2 w-full max-w-[90rem] px-6 sm:px-12 lg:px-24 2xl:max-w-[112.5rem] min-[120rem]:max-w-[137.5rem]"
				initial={{ opacity: 0 }}
				animate={{ opacity: 1 }}
