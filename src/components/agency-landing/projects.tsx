...
function ProjectItem({
	...
	useEffect(() => {
		...
		xTo.current = gsap.quickTo(canvasWrapperRef.current, "x", {
			duration: 0.8,
			ease: "power3.out",
		});
		yTo.current = gsap.quickTo(canvasWrapperRef.current, "y", {
			duration: 0.8,
			ease: "power3.out",
		});
		scaleTo.current = gsap.quickTo(canvasWrapperRef.current, "scale", {
			duration: 0.6,
			ease: "power2.out",
		});
	}, []);
	...
	return (
		...
		>
			<div className="mx-auto max-w-[90rem] px-6 sm:px-12 lg:px-24 2xl:max-w-[112.5rem] min-[120rem]:max-w-[137.5rem]">
				<div
					className={`flex flex-col gap-8 ${isEven ? "md:flex-row" : "md:flex-row-reverse"} md:items-center md:gap-16`}
				>
					{/* biome-ignore lint/a11y/noStaticElementInteractions: hover tracking for visual effect only */}
					<div
					...
					<div
						className={`flex flex-col md:w-2/5 ${isEven ? "" : "md:text-right"}`}
					>
						...
						<p
							ref={descRef}
							className={`text-muted-foreground text-xl leading-relaxed ${isEven ? "max-w-lg" : "max-w-lg md:ml-auto"}`}
						>
							{project.description}
						</p>
					</div>
	...
	return (
		...
		<section id="projects" className="projects bg-background relative py-24">
			...
			<div className="pb-16">
				...
				>
					Signature <span className="font-serif font-thin">Stories</span>&nbsp;
				</VelocityText>
			</div>
