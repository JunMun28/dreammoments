...
describe("createRateLimiter", () => {
	...
	test("allows requests within limit", () => {
		const limiter = createRateLimiter("test1", { maxAttempts: 3, windowMs: 60000 });
	...
	test("blocks requests over limit", () => {
		const limiter = createRateLimiter("test2", { maxAttempts: 2, windowMs: 60000 });
	...
	test("tracks different keys independently", () => {
		const limiter = createRateLimiter("test4", { maxAttempts: 2, windowMs: 60000 });
	...
	test("different limiters have separate stores", () => {
		const limiter1 = createRateLimiter("store1", { maxAttempts: 2, windowMs: 60000 });
		const limiter2 = createRateLimiter("store2", { maxAttempts: 2, windowMs: 60000 });
	...
	test("handles many requests correctly", () => {
		const limiter = createRateLimiter("test7", { maxAttempts: 100, windowMs: 60000 });
